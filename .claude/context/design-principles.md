...
- [ ] **Define Spacing Units:**
  ...
  - [ ] **Footer Button Spacing:** Use `gap-4` (16px) for horizontal spacing between action buttons in footers, dialogs, and forms. This provides comfortable spacing for touch targets and visual clarity.
    - Page-level form footers: `flex items-center gap-4`
    - AlertDialogFooter: `sm:space-x-4`
    - CardFooter: Add `gap-4` when containing buttons
- [ ] **Define Border Radii:**
...
### C. Advanced Query Builder for List Pages

- [ ] **Universal Pattern:** ALL list pages (customers, invoices, stock, banking, cases, etc.) MUST implement the same query builder pattern for consistency.
- [ ] **Progressive Disclosure:** Query builder is hidden by default, revealed via toggle button.
- [ ] **Toggle Button Placement:**
  - [ ] Icon: `SlidersHorizontal` from lucide-react
  - [ ] Position: Inside search input field, absolute positioning at `right-10`
  - [ ] Visual feedback: Highlighted with `text-primary` when active
  - [ ] Accessibility: Clear title attribute describing the toggle action
- [ ] **Visual Integration:**
  - [ ] Smooth slide-in animation: `animate-in slide-in-from-top-2`
  - [ ] Consistent styling with shadcn/ui components
  - [ ] Dark mode support for all select dropdowns and inputs
  - [ ] Visual hierarchy: Query builder appears below filters, above data table
- [ ] **Component Selection:**
  - [ ] Use `react-querybuilder` library for core functionality
  - [ ] Custom shadcn/ui Select components for field/operator/value selectors
  - [ ] Branch lines enabled for visual clarity of rule hierarchy
  - [ ] Drag-and-drop support for reordering rules
- [ ] **User Experience:**
  - [ ] Combinators (AND/OR) clearly labeled between rules
  - [ ] Add Rule / Add Group buttons prominently displayed
  - [ ] Remove buttons (×) for each rule clearly visible
  - [ ] All controls must respect dark mode
  - [ ] Query builder scrolls WITH the data table (unified scrolling)
- [ ] **Translation Support:**
  - [ ] All keywords, operators, field labels translated via i18n
  - [ ] Support for Turkish, English, and Arabic
  - [ ] Dynamic field labels based on entity type (customers, invoices, etc.)
- [ ] **State Management:**
  - [ ] Query state persisted and passed to data fetching
  - [ ] Integrates with existing filter state
  - [ ] Clear filters button resets both simple filters AND query builder
  - [ ] Query builder state visible in URL params (optional enhancement)

### D. Configuration Panels Module (Microsite, Admin Settings)
